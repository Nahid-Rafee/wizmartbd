        ...
        <form class="" action="" method="GET">
            <div class="row gutters-10">
                <div class="form-group col-md-3">
                    <input class="form-control form-control-sm" type="text" name="keyword" id="keyword"  placeholder="Search by Product Name/Barcode">
                </div>
                <div class="form-group col-md-3">
                    <select name="poscategory" class="form-control form-control-sm aiz-selectpicker" data-live-search="true">
                ...
                <div class="form-group col-md-3">
                    <select name="brand"  class="form-control form-control-sm aiz-selectpicker" data-live-search="true">
                ...
                <div class="col-md-3">
                    <button type="button" class="btn btn-primary" onclick="filter_search_product()">{{translate('Search')}}</button>
                </div>
                    ...
                    <div class="card">
                         <!-- <div class="card-header d-block">
                           <div class="form-group">
                            ...
                            <div class="row gutters-5">
                             ...
                             <div class="form-group mt-10" style="margin:10px;">
                        ...
                        <div class="card-body">
                            <div class="aiz-pos-product-list c-scrollbar-light">
                                    ...
                                    <div class="row gutters-0" id="product-list">
                                ...
                                <div id="load-more" >
                                    <a class="text-center fs-14 fw-600 p-2 bg-soft-primary c-pointer" onclick="loadMoreProduct()">Load More</a>
                                </div>
                            ...
                            <div class="d-flex">
                                <div class="flex-grow-1">
                                    <select name="user_id" class="form-control form-control-sm aiz-selectpicker pos-customer" data-live-search="true" onchange="getShippingAddress()">
                                        ...
                                        @foreach (App\Models\User::all() as $key => $customer)
                                            @if ($customer->name)
                                        ...
                                        @if (Session::has('posCart'))
                                            @forelse (Session::get('posCart') as $key => $cartItem)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <button type="button" class="btn btn-circle btn-icon btn-sm btn-danger" onclick="removeFromCart({{ $key }},{{$cartItem['quantity']}})">
                                ...
                                <div class="dropdown mr-3 dropup">
                                    ...
                                    <div class="dropdown-menu p-3 dropdown-menu-lg">
                                        ...
                                        <div class="input-group">
                                            <input type="number" min="0" placeholder="Amount" name="shipping" class="form-control" value="{{ Session::get('shipping', 0) }}" required onchange="setShipping()">
...
@section('modal')
    ...
    <div id="order-confirm" class="modal fade" data-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom">
            <div class="modal-content" id="variants">
                ...
                <div class="modal-footer">
                    <button type="button" id="cancelOrderButton" class="btn btn-styled btn-base-3" data-dismiss="modal">{{translate('Close')}}</button>
                    <button type="button" id="confirmOrderButton" onclick="submitOrder('cash')" class="btn btn-styled btn-base-1 btn-primary">{{translate('Comfirm Order')}}</button>
                </div>
            </div>
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function filterProducts(){
            ...
            $.get('{{ route('pos.search_product') }}',{keyword:keyword}, function(data){
                $('#product-list').html('');
                setProductList(data);
            });

        }
        function filter_search_product()
        {
        ...
        function loadMoreProduct(){
            ...
            if(products != null && products.next_page_url != null){
                $.get(products.next_page_url,{}, function(data){
                    setProductList(data);
                });
            }
        }
        ...
        function setProductList(data){
            products = data;
            var currentProducts = data.data;
            for (var i = 0; i < currentProducts.length; i++) {
                $('#product-list').append('<div class="col-3">' +
                    '<div class="card bg-light c-pointer mb-2 product-card" data-id="'+currentProducts[i].id+'" >'+
                        '<span class="absolute-top-left bg-dark text-white px-1">৳'+currentProducts[i].unit_price +'</span>'+
                        '<img src="'+public_url+'/'+currentProducts[i].file_name+'"  class="card-img-top img-fit h-100px mw-100 mx-auto" >'+
                        '<div class="card-body p-2">'+
                            '<div class="text-truncate-2 small">'+ currentProducts[i].name +'</div>'+
                        '</div>'+
                    '</div>'+
                '</div>');
            }
        ...
        function removeFromCart(key,quantity){
            // $.post('{{ route('pos.removeFromCart') }}', {_token:'{{ csrf_token() }}', key:key,quantity:quantity}, function(data){
        ...
        function updateQuantity(key){
            fetch('{{ route('pos.updateQuantity') }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        function getShippingAddress(){
            ...
            fetch('{{ route('pos.getShippingAddress') }}?_token={{ csrf_token() }}&id=' + $('select[name=user_id]').val())
            .then(response => response.text())
            .then(data => {
            ...
            fetch('{{ route('pos.order_place') }}', {
                ...
                .then(data => {
                    if(data && data == 1){
                        ...
                        // alert("Order is not successful.");
                        Swal.fire({
                        title: 'Order is not successful.',
                        confirmButtonText: 'OK!',
                        }).then((result) => {
                            if (result.isConfirmed) {
                            window.location.reload();
